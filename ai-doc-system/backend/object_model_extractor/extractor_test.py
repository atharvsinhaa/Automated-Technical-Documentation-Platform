                    ...
                    if edge.relation == KGRelationType.CONTAINS:
                        ...
                        if method_node and method_node.node_type in (KGNodeType.METHOD, KGNodeType.FUNCTION):
                            methods.append(LLDMethod(
                                ...
                                signature=f"{method_node.name}({', '.join(method_node.params)}) -> {method_node.return_type or 'untyped'}",
                            ...
                            if child.node_type == KGNodeType.CONSTRUCTOR:
                                ...
                                for p in child.params:
                                    if ":" in p:
                                        ...
                                        if p_type and p_type not in ("str", "int", "float", "bool", "dict", "list", "set", "untyped", "None") and p_type != cls.name:
                                ...
                                methods.append(LLDMethod(
                                    name=child.name,
                                    signature=f"{child.name}({', '.join(child.params)}) -> {child.return_type or 'untyped'}",
                                    description=child.docstring,
                                    parameters=child.params,
                                ...
                                fields.append(f"{child.name}: {child.return_type or 'untyped'}")
                                if child.return_type and child.return_type not in ("str", "int", "float", "bool", "dict", "list", "set", "untyped", "None"):
