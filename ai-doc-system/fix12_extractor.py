...
# Replace the "Any" with "untyped" broadly
code = code.replace("'Any'", "'untyped'")
code = code.replace('"Any"', '"untyped"')

# FIX 2 & 3: Field types and method signatures
...
code = re.sub(r'    def _extract_classes_and_interfaces\(self, ir: SemanticIR, kg: Optional\[KnowledgeGraph\]\) -> Tuple\[List\[LLDClass\], List\[LLDInterface\]\]:.*?return classes, interfaces', classes_method, code, flags=re.DOTALL)


# FIX 4: Sequence flows
sf_method = """    def _extract_sequence_flows(self, ir: SemanticIR, kg: Optional[KnowledgeGraph] = None) -> List[LLDSequenceFlow]:
        ...
        if getattr(ir, 'request_flows', None):
            for i, flow in enumerate(ir.request_flows):
                ...
                # Exclude if all steps are SQL table names
...
mod_method = """    def _extract_modules(self, ir: SemanticIR, components: List[LLDComponent]) -> List[LLDModule]:
        ...
        return modules"""
code = re.sub(r'    def _extract_modules\(self, ir: SemanticIR, components: List\[LLDComponent\]\) -> List\[LLDModule\]:.*?return modules', mod_method, code, flags=re.DOTALL)

# FIX 12: SQL Table Columns
db_method = """    def _parse_create_table_columns(self, sql_file_path, table_name) -> list:
        import re
        try:
    ...
    def _extract_database_objects(self, ir: SemanticIR, kg: Optional[KnowledgeGraph]) -> List[LLDDatabaseObject]:
        ...
        return tables"""
code = re.sub(r'    def _extract_database_objects\(\s*self, ir: SemanticIR, kg: Optional\[KnowledgeGraph\]\s*\) -> List\[LLDDatabaseObject\]:.*?return \w+', db_method, code, flags=re.DOTALL)

# Add class deduplication to extract
dedup_logic = """
