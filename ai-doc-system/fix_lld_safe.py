...
generate_method = """    def generate(
        ...
        self._section_component_architecture(lines, model, dp.get("component_architecture_diagram") or dp.get("lld_component_architecture_diagram"))
        self._section_class_design(lines, model)
        self._section_sequence_diagrams(lines, model, dp.get("sequence_diagram") or dp.get("lld_sequence_diagram"))
        self._section_api_specifications(lines, model)
        self._section_data_model(lines, model)
        self._section_database_design(lines, model, dp.get("erd_diagram") or dp.get("lld_erd_diagram"))
        self._section_dependency_architecture(lines, model, dp.get("dependency_diagram") or dp.get("lld_dependency_diagram"))
        ...
        self._section_deployment_units(lines, model, dp.get("deployment_unit_diagram") or dp.get("lld_deployment_unit_diagram"))
        
        return "\\n".join(lines)"""
code = re.sub(r'    def generate\(.*?return full_doc', lambda _: generate_method, code, flags=re.DOTALL)
...
seq_method = """    def _section_sequence_diagrams(self, lines: List[str], model: LLDModel, mmd_code=None):
        ...
        for sf in model.sequence_flows:
            ...
            lines.append("")"""
code = re.sub(r'    def _section_sequence_diagrams\(self, lines: List\[str\], model: LLDModel, dp: Dict\[str, str\]\):.*?def _section_api_specifications', lambda _: seq_method + "\n\n    def _section_api_specifications", code, flags=re.DOTALL)
...
db_method = """    def _section_database_design(self, lines: List[str], model: LLDModel, mmd_code):
        ...
        for db in model.database_objects:
            ...
            lines.append("")"""
code = re.sub(r'    def _section_database_design\(self, lines: List\[str\], model: LLDModel, mmd_code\):.*?def _section_dependency_architecture', lambda _: db_method + "\n\n    def _section_dependency_architecture", code, flags=re.DOTALL)
...
dep_method = """    def _section_dependency_architecture(self, lines: List[str], model: LLDModel, mmd_code):
        ...
        if circular:
            ...
            lines.append("")"""
code = re.sub(r'    def _section_dependency_architecture\(self, lines: List\[str\], model: LLDModel, mmd_code\):.*?def _section_external_integrations', lambda _: dep_method + "\n\n    def _section_external_integrations", code, flags=re.DOTALL)
