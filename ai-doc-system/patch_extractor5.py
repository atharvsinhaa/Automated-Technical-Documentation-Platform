...
# Remove the methods from the bottom
to_remove = """    def _extract_circular_dependencies(self, ir, kg, deps):
...
# Insert before `def _infer_sql_type`
code = code.replace("def _infer_sql_type", to_remove + "\n\ndef _infer_sql_type")

with open("backend/object_model_extractor/extractor.py", "w") as f:
    f.write(code)

print("Moved methods to correct scope.")
